...
//		$query->select('DISTINCT a.name as label, a.id as value');
		$query->select("DISTINCT CONCAT(`last_name`, ', ', `first_name`, IF((ISNULL(`kennel_name`) or `kennel_name` = ''), '', CONCAT(' (', `kennel_name`, ')'))) as label, a.`id` as value ");
			...
			if (!empty($value)) {
				switch ($name) {
					...
					case 'name' :
						...
						$query->where("WHERE LCASE(CONCAT(`last_name`, ', ', `first_name`, IF((`kennel_name` = ''), '', CONCAT(' (', `kennel_name`, ')')))) LIKE '%".mysql_real_escape_string(strtolower($query))."%' ");
						break;
					// Typical =
					default :
						break;				
