...
if($this->item->id) {
	$canState = JFactory::getUser()->authorise('core.edit.state','com_pedigree.judging');
} else {
	$canState = JFactory::getUser()->authorise('core.edit.state','com_pedigree.judging.'.$this->item->id);
}
?>
</style>
<script type="text/javascript">
    getScript('//ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.min.js', function() {
        jQuery(document).ready(function() {
            jQuery('#form-judging').submit(function(event) {
                
            });

            
			jQuery('input:hidden.id_person').each(function(){
				var name = jQuery(this).attr('name');
				if(name.indexOf('id_personhidden')){
					jQuery('#jform_id_person option[value="' + jQuery(this).val() + '"]').attr('selected',true);
				}
			});
					jQuery("#jform_id_person").trigger("liszt:updated");
...
<div class="judging-edit front-end-edit">
    ...
    <form id="form-judging" action="<?php echo JRoute::_('index.php?option=com_pedigree&task=judging.save'); ?>" method="post" class="form-validate form-horizontal" enctype="multipart/form-data">
	...
	<?php foreach((array)$this->item->id_person as $value): ?>
		<?php if(!is_array($value)): ?>
			<input type="hidden" class="id_person" name="jform[id_personhidden][<?php echo $value; ?>]" value="<?php echo $value; ?>" />';
		<?php endif; ?>
	<?php endforeach; ?>
	<div class="control-group">
		...
		<div class="controls"><?php echo $this->form->getInput('created_by'); ?></div>
                ...
                <script type="text/javascript">
                    ...
                    jQuery('.tab-pane select').each(function(){
                       ...
                       document.getElementById("form-judging").appendChild(input);
                    });
                </script>
             <?php endif; ?>
        <div class="control-group">
            <div class="controls">
                ...
                <a class="btn" href="<?php echo JRoute::_('index.php?option=com_pedigree&task=judgingform.cancel'); ?>" title="<?php echo JText::_('JCANCEL'); ?>"><?php echo JText::_('JCANCEL'); ?></a>
            </div>
        </div>
        
        <input type="hidden" name="option" value="com_pedigree" />
        <input type="hidden" name="task" value="judgingform.save" />
